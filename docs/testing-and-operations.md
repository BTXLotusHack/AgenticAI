...
The automated checks cover health, missing authentication, join and readiness, contract/media/body rejection, CORS and WebSocket-origin denial, browser-compatible fixture authentication, nonmember/member-channel authorization, membership revalidation, deadlines, coordinate-free logs and realtime graph delivery. They also submit the canonical duplicate, stale, offline, weak, split and reconnect telemetry through HTTP, approve POI001, and verify the generated summary has one resolved split, eight notifications and a `900 m` maximum confirmed gap. Fixture credentials and in-memory state are restricted to local/test use and do not substitute for Cognito, DynamoDB or deployed AWS smoke tests.
