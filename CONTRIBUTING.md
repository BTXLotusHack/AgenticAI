...
3. Fetch `origin`, update `main` using fast-forward only, and create a short-lived feature branch from `origin/main`.
4. For architectural changes, add an ADR under `docs/adr/`.
5. For safety-policy changes, describe evidence, affected scenarios, rollout and rollback.
6. Keep the unit of work small enough to verify independently.

## Branching workflow

Loopin uses a single integration branch: `main`.
...
- Pull requests target `main`; protected environments and explicit approvals control deployment promotion instead of a separate Git branch.
